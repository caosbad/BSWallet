...
Mixin.prototype = {
  createUser: function(callback, privateKey, publicKey, pin, name) {
    const self = this;
    publicKey = this.strip(publicKey);
    let params = {
      session_secret: publicKey,
      full_name: name || uuid().toLowerCase()
    };
    self.api.request('POST', '/users', params, function(resp) {
      ...
      const result = { ...resp.data, privateKey, pin };
      self.updatePIN(
        function(resp) {
  ...
  importWallet: function(callback, data) {
    ...
    const token = this.api.signAuthenticationToken(
      userId,
      sessionId,
      privatekey,
      method,
  ...
  snapshots: function(callback, offset, limit) {
    ...
    this.api.request(
      'GET',
      '/snapshots?limit=' + limit + '&offset=' + offset,
      undefined,
      function(resp) {
        return callback(resp);
      }
  ...
  encryptedPin: function(pin, pinToken, sessionId, privateKey, iterator) {
    ...
    let cipher = crypto.createCipheriv(
      'aes-256-cbc',
      this.hexToBytes(forge.util.binary.hex.encode(pinKey)),
      iv16
    );
